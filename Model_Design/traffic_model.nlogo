...
to go
  ;; change the light first / passive-switch
  if (ticks - last-change-tick) >= min-switch-period [
    passive-switch
    set last-change-tick ticks
  ]
  signal-switch
  ...
  ;; calculate data collector
  set avg-wait-time (sum [wait-time] of cars with [speed = 0] / (count cars with [speed = 0] + 1))
  set avg-speed (sum [speed] of cars with [speed > 0] / (count cars with [speed > 0] + 1))
  tick
end

;; change the signal based (may use the button?)
to signal-switch
  ifelse NS? [
...
to move
  ...
  repeat speed [
    ...
    if not can-move? 1 [
      set reward reward + tips ;; give the tips to signal
      die
...
;; passive-switch when is too long
to passive-switch
  ifelse NS? [
    set NS? false
  ] [
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count cars with [ speed = 0 ]"
"W2E" 1.0 0 -2674135 true "" "plot count cars with [ heading = 90 and speed = 0 ]"
"N2S" 1.0 0 -13791810 true "" "plot count cars with [ heading = 180 and speed = 0 ]"
"E2W" 1.0 0 -955883 true "" "plot count cars with [ heading = 270 and speed = 0 ]"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot avg-speed"
